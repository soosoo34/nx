...
describe('downloadTemplate', () => {
  ...
  it('throws NETWORK_ERROR with a --preset=empty hint when github.com is unreachable', async () => {
    ...
    await expect(
      ...
      code: 'NETWORK_ERROR',
      message: expect.stringMatching(/github\.com is not reachable/),
    });
    await expect(
      downloadTemplate('nrwl/empty-template', join(tmpDir, 'proj'))
    ).rejects.toMatchObject({
  ...
  it('throws NETWORK_ERROR with the hint for HTTP 403 (egress blocked by a sandbox proxy)', async () => {
    globalThis.fetch = jest.fn(() =>
      Promise.resolve({ ok: false, status: 403, body: null })
    ) as any;
  ...
  it('throws TEMPLATE_CLONE_FAILED without the hint for HTTP errors (missing repo or branch)', async () => {
    globalThis.fetch = jest.fn(() =>
