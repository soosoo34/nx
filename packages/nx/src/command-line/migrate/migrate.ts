...
export function readLocalNxVersion(root: string): string | undefined {
  ...
  // preferring the manifest would judge a dead install a completed switch
  // and refuse persistently. The known misread left is yarn classic
  // driving a workspace with a leftover Berry manifest and no other
  // lockfile; telling the two yarns apart from disk is not worth the
  // fragility, and a `yarn node` runtime probe was rejected because its
  // stdout is forgeable through an inherited NODE_OPTIONS preload and a
  // hung descendant holding the output pipe outlives a subprocess timeout
